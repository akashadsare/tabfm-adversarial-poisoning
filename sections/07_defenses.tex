...
As shown in Figure \ref{fig:defense}, dropping between 30\% and 50\% of the context rows completely breaks the adversarial gradient alignment (reducing the Adversarial MSE from 1.9691 back down to 0.76). Counterintuitively, this subsampling actually improved the baseline MSE on clean data (from 0.5227 down to 0.49). We hypothesize this occurs because randomly dropping rows acts as a form of stochastic regularization, preventing the Set Transformer from overfitting to natural noise within the small 50-row context window. Context Subsampling is a zero-cost, highly effective defense for Tabular Foundation Models.
